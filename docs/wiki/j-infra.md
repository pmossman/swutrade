...
**`vercel.json`** — Rewrites + a `crons` block. One Vercel cron today: `/api/cron/signals` (daily at 08:00 UTC, expires past-due `card_signals` posts). The pretty URL rewrites to `api/bot?action=cron-signals`; there is no `api/cron/` directory, the bot dispatcher owns the cron handler. Vercel Hobby caps cron frequency at daily, so anything more frequent lives on GitHub Actions: `.github/workflows/refresh-prices.yml` (every 2h, fires a Vercel deploy hook) and `.github/workflows/session-followups.yml` (every 5 min, hits `/api/cron/session-followups` directly with `Authorization: Bearer ${CRON_SECRET}`). Both routing rewrites stay in `vercel.json` — only the schedule moves out of band. Rewrites are ordered: pretty-URLs first, catch-all `"^/api/(.*)"` last, so a single-purpose endpoint (`api/search.ts`, `api/og.ts`, `api/popular-wants.ts`) stays reachable without an explicit rewrite. The signals dispatcher's pretty URLs (`/api/signals/create`, `/cancel`, `/mine`, `/seed`) all rewrite to `api/signals?action=…`.
