...
**`vercel.json`** — Rewrites + a `crons` block (one daily cron at `/api/cron/signals` at 08:00 UTC). The cron rewrite points the pretty URL at `api/bot?action=cron-signals` — there is no `api/cron/` directory; the bot dispatcher owns the cron handler. Rewrites are ordered: pretty-URLs first, catch-all `"^/api/(.*)"` last, so a single-purpose endpoint (`api/search.ts`, `api/og.ts`, `api/popular-wants.ts`) stays reachable without an explicit rewrite. The signals dispatcher's pretty URLs (`/api/signals/create`, `/cancel`, `/mine`, `/seed`) all rewrite to `api/signals?action=…`. The price-refresh job is a separate GitHub Actions workflow (not in `vercel.json`).
