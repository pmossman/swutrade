...
**`vercel.json`** — Rewrites + a `crons` block. One Vercel cron today: `/api/cron/signals` (daily at 08:00 UTC, expires past-due `card_signals` posts). The pretty URL rewrites to `api/bot?action=cron-signals`; there is no `api/cron/` directory, the bot dispatcher owns the cron handler. Vercel Hobby caps cron frequency at daily, so anything more frequent lives elsewhere: `.github/workflows/refresh-prices.yml` (every 2h, fires a Vercel deploy hook) for the price-refresh build trigger, and **Inngest** for the session-followups sweep (every 5 min). Rewrites are ordered: pretty-URLs first, catch-all `"^/api/(.*)"` last, so a single-purpose endpoint (`api/search.ts`, `api/og.ts`, `api/popular-wants.ts`) stays reachable without an explicit rewrite. The signals dispatcher's pretty URLs (`/api/signals/create`, `/cancel`, `/mine`, `/seed`) all rewrite to `api/signals?action=…`.

**Inngest** — scheduled functions + durable-workflow runtime, picked over Vercel Pro / GitHub Actions for app-level schedules. Setup is three files: `lib/inngest/client.ts` (the Inngest client), `lib/inngest/functions.ts` (function declarations — today just `sessionFollowupsCron`), and `api/inngest.ts` (the serve handler at `/api/inngest`, using the `inngest/express` adapter — the closest match to Vercel Functions, since Inngest doesn't ship a dedicated Vercel adapter). Auth: `INNGEST_EVENT_KEY` + `INNGEST_SIGNING_KEY` env vars on Vercel; the dashboard polls `/api/inngest` after each deploy to sync function definitions. Free tier covers 50k function runs/month — at the current 5-min cadence we'd use ~8.6k/month. Adding a new scheduled job is a one-line addition to `lib/inngest/functions.ts` plus appending to the `functions: [...]` array in `api/inngest.ts`.
