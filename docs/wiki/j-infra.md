...
- **Drizzle journal** — `drizzle/meta/_journal.json` is the ordered migration manifest. Version 7, PostgreSQL dialect; the tail moves with each shipped migration. Recent additions: `0029_drop_proposals` (Phase C, sessions become the only trade primitive), `0030_add_dm_session_activity` (auto-DM-on-activity column + `last_notified_at` jsonb wired up), `0031_prefs_hygiene_drop_dead_columns` (peer scope + dead pref columns retired), `0032_drop_last_notified_at` (retired alongside the session-followups cron; see Inngest section below).
...
**`vercel.json`** — Rewrites + a `crons` block. One Vercel cron today: `/api/cron/signals` (daily at 08:00 UTC, expires past-due `card_signals` posts). The pretty URL rewrites to `api/bot?action=cron-signals`; there is no `api/cron/` directory, the bot dispatcher owns the cron handler. Vercel Hobby caps cron frequency at daily, so the price-refresh build trigger lives elsewhere: `.github/workflows/refresh-prices.yml` (every 2h, fires a Vercel deploy hook). Rewrites are ordered: pretty-URLs first, catch-all `"^/api/(.*)"` last, so a single-purpose endpoint (`api/search.ts`, `api/og.ts`, `api/popular-wants.ts`) stays reachable without an explicit rewrite. The signals dispatcher's pretty URLs (`/api/signals/create`, `/cancel`, `/mine`, `/seed`) all rewrite to `api/signals?action=…`.

**No Inngest** — there used to be one Inngest scheduled function (`sessionFollowupsCron`, every 5 min) that swept `trade_sessions` and DM'd participants about counterpart activity. Retired 2026-06-04 after the sweep — combined with Inngest's auto-retry on failure — blew through Neon's free-tier data-transfer quota and burned ~17h of Vercel Active CPU in a retry storm. The DM-on-counterpart-activity feature is intentionally gone; users discover activity by opening the app or via the next `/trade` Discord interaction. See [[project-inngest-retry-storm]] memory for the full incident. Re-introducing app-level schedules in the future: prefer event-driven (`inngest.send` from the write path → debounced consumer) over polling-style cron, and set `retries: 0` on any DB-bound function so a single failure doesn't amplify into N retries.
...
Current `api/*.ts` files at the top level: `auth.ts`, `bot.ts`, `me.ts`, `og.ts`, `popular-wants.ts`, `search.ts`, `sessions.ts`, `signals.ts`, `sync.ts`, `trades.ts` plus the `api/user/[handle].ts` dynamic route = **11 functions** (plus `_fonts.ts` which is imported by `og.ts` and not a function in its own right — the underscore-prefix convention marks it non-routable). `api/context.md:9` mentions a 12-function Hobby cap as the historical driver. If headroom has improved on the current plan tier, it's still worth consolidating new surfaces into existing dispatchers rather than finding out the hard way. Any greenfield endpoint should start inside `api/me.ts` (authenticated user ops), `api/sessions.ts` (session ops), or `api/signals.ts` (signal ops) — or carve out a new dispatcher only when the shape genuinely doesn't fit.
