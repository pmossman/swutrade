...
**`vercel.json`** — Rewrites + a `crons` block. Two crons today: `/api/cron/signals` (daily at 08:00 UTC, expires past-due `card_signals` posts) and `/api/cron/session-followups` (every 2 min, sends catch-up DMs for unread counterpart activity in active sessions — see [`a-sessions.md`](./a-sessions.md)). Both pretty URLs rewrite to `api/bot?action=cron-…`; there is no `api/cron/` directory, the bot dispatcher owns both cron handlers. Rewrites are ordered: pretty-URLs first, catch-all `"^/api/(.*)"` last, so a single-purpose endpoint (`api/search.ts`, `api/og.ts`, `api/popular-wants.ts`) stays reachable without an explicit rewrite. The signals dispatcher's pretty URLs (`/api/signals/create`, `/cancel`, `/mine`, `/seed`) all rewrite to `api/signals?action=…`. The price-refresh job is a separate GitHub Actions workflow (not in `vercel.json`).
