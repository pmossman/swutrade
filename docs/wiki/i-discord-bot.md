...
- ~~`cron-session-followups`~~ retired 2026-06-04. The cron + its Inngest scheduler + the entire `performSessionFollowupsSweep` machinery were deleted after the sweep blew Neon's data-transfer quota and a retry storm burned ~17h of Vercel Active CPU. The DM-on-counterpart-activity feature no longer exists; lifecycle DMs (invite, declined, settled) keep their synchronous paths. See [[project-inngest-retry-storm]] memory + [`j-infra.md`](./j-infra.md) for the post-mortem.
