...
- **`TRADES_CHANNEL_ID`** — env var naming the guild text channel where private trade threads get created. When unset (or unreachable), proposals fall back to per-user DMs. Auto-discovered on install as `#swutrade-threads` (see `botInstalledGuilds.tradesChannelId`).
...
- **Channel/webhook id sprawl.** `TRADES_CHANNEL_ID`, `DISCORD_ERROR_WEBHOOK_URL`, `DISCORD_RELEASE_WEBHOOK_URL` all live in Vercel env. The `bot_installed_guilds.trades_channel_id` per-guild cache is the right pattern; the global envs exist because SWUTrade's own install predates the auto-create flow. Eventually the global `TRADES_CHANNEL_ID` should go away and callers should always look up per-guild. Low priority — works fine today.
