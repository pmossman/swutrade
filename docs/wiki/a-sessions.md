...
**`lib/schema.ts`** (lines 400–500) — `tradeSessions` table + `sessionEvents` append-only log. Partial unique index, FK policies (cascade on session, set-null on event actor) all defined here.
...
| `last_edited_by_user_id` | `text NULL, FK users(id) ON DELETE SET NULL` | bookkeeping; previously read by the retired session-followups cron |
...
- **Activity DMs (chat / edit / confirm / suggestion-*) are intentionally gone as of 2026-06-04.** Earlier iterations tried synchronous DMs with cooldowns (`notifySessionActivity`, retired 2026-05-08), then an Inngest cron sweep that scanned `trade_sessions WHERE status='active'` every 5 min. The sweep — combined with Inngest's auto-retry-on-failure — exploded our Neon free-tier data-transfer quota and burned ~17h of Vercel Active CPU in a retry storm. The entire `performSessionFollowupsSweep` machinery + the `last_notified_at` JSONB column were deleted. Users discover counterpart activity by opening the app or via the next `/trade` Discord interaction. Lifecycle DMs (invite, declined, settled) keep their synchronous paths because they're user-actioned moments. See [[project-inngest-retry-storm]] memory for the post-mortem. If we ever want to bring activity DMs back: event-driven (`inngest.send` from the write path → debounced consumer) over polling-style cron, with `retries: 0` on any DB-bound function.
