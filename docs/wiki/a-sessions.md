...
- **`last_notified_at` jsonb interlocks auto-DM cooldowns**. Written by `notifySessionActivity` (`lib/sessions.ts`) when a chat / edit / confirm / suggest fires a counterpart DM; the 10-minute cross-flow cooldown reads from here so back-to-back activity events don't spam. Replaces the old manual Ping button (removed 2026-05). Other long-tail "we should DM you" jobs can plug into the same column with a per-job key.
