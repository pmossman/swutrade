...
- **`last_notified_at` jsonb is the cron's "have we DM'd this user about this activity yet" record**. Written by `performSessionFollowupsSweep` (`lib/sessionFollowups.ts`) when the recipient gets a catch-up DM about counterpart-authored chat / edit / confirm / suggestion-* events. The sweep is triggered every 5 minutes by Inngest (`lib/inngest/functions.ts::sessionFollowupsCron`); the HTTP cron endpoint at `/api/cron/session-followups` (handled in `api/bot.ts::runSessionFollowupsSweep`) is a manual-trigger escape hatch but isn't on a schedule. The scheduler interval IS the cooldown — there's no 10-min timer to interlock against, no read-state-resets-cooldown gating, no synchronous fire-then-debounce. Replaces the old `notifySessionActivity` synchronous DM (retired 2026-05-08) and the manual Ping button (removed 2026-05-06). Other long-tail "we should DM you" jobs can plug into the same column with a per-job key. Why Inngest and not Vercel cron: Vercel Hobby caps cron frequency at daily; Inngest's free tier (50k function runs/month) covers our 5-min cadence ~6× over without coupling app behaviour to CI infrastructure. See [`j-infra.md`](./j-infra.md) for the Inngest setup.
