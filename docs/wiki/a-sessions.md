...
- **`last_notified_at` jsonb is the cron's "have we DM'd this user about this activity yet" record**. Written by `runSessionFollowupsSweep` (`api/bot.ts`, action `cron-session-followups`, every 2 min) when the recipient gets a catch-up DM about counterpart-authored chat / edit / confirm / suggestion-* events. The cron interval IS the cooldown — there's no 10-min timer to interlock against, no read-state-resets-cooldown gating, no synchronous fire-then-debounce. Replaces the old `notifySessionActivity` synchronous DM (retired 2026-05-08) and the manual Ping button (removed 2026-05-06). Other long-tail "we should DM you" jobs can plug into the same column with a per-job key.
