...
>
> Files covered:
> - `src/components/CollectionView.tsx` — unified tabbed surface for wants + available; merged 2026-05-21 from the previously-separate WishlistView + BinderView
...
**`src/components/CollectionView.tsx`** — unified tabbed surface for both lists, mounted at `?view=wishlist` (defaults to Wishlist tab) and `?view=binder` (defaults to Trade Binder tab). Reached from Home's Wishlist + Binder modules ("Open wishlist" / "Open trade binder"), the NavMenu's "My Wishlist" + "My Trade Binder" entries, and direct URL. Renders AppHeader + breadcrumb ("Home › Wishlist" or "Home › Trade binder") + tab bar (Wishlist + Trade binder, count badges per tab) + per-tab share affordances ("Copy link" + "Share image" — both scoped to the active tab so a wishlist share carries only `?w=`, binder share only `?a=`) + the appropriate `<WantsPanel>` / `<AvailablePanel>` body. Tab is URL-synced via `?tab=`; switching uses `replaceState` so the back button stays useful. Merged 2026-05-21 from the previously-separate WishlistView + BinderView — parker flagged that the physical-collection audit workflow (card in hand → "do I want this?" → "do I have extras?") was bouncing users back through Home to switch lists.

**`src/components/lists/WantsPanel.tsx`** — shared list+picker body for wants. Owns its own `mode` (`list` / `picker`) and `editingWantId` state so both callers (drawer + CollectionView's wishlist tab) can render it without coordinating. Priority-first sort mirrors Home's WishlistModule so "top of the list" is consistent across every surface. Accepts an `emptyState` override so the drawer's "No wants yet" message and the dedicated view's "Your wishlist is empty" message stay distinct.
...
- **Wishlist + Binder MERGED into CollectionView (2026-05-21)** — partial walkback of the 2026-04-21 split. parker's audit workflow ("physical card in hand → want it? have extras?") was bouncing users back through Home to switch lists. The two dedicated views were 80% identical chrome anyway. Collapsed into one tabbed `CollectionView` at `?view=wishlist` / `?view=binder` (both URLs preserved for backward compat; the routing layer picks the `defaultTab` based on which one was hit) with internal `?tab=` for free tab switching. Per-tab state (toolbar filters, selection, etc.) still persists via the existing per-surface localStorage keys — the merge is at the view level, not the data level. The data-model insight from the 2026-04-21 entry (wants has priority + restriction; available has neither) still holds — the panels `<WantsPanel>` + `<AvailablePanel>` stay distinct and the tab bar swaps between them. Home modules' "Edit wishlist →" / "Edit trade binder →" text links replaced with proper "Open wishlist" / "Open trade binder" buttons + an icon-only share button next to each (the prior text-link pair had touch-target collision on mobile).
