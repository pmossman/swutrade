...
- **Delivery cascade** — on `propose`, the backend prefers a private Discord thread when `TRADES_CHANNEL_ID` is set AND both parties' `communicationPref` agrees; otherwise it DMs the recipient with a `Request thread` button conditionally attached. Decision matrix in `lib/threadConsent.ts:48-61`.
...
3. **Delivery cascade.** `handlePropose` resolves both sides' `communicationPref` through `resolvePref` (cascade: peer override → self default → registry default) and passes the pair to `deliveryForPair` (`lib/threadConsent.ts:48`). The result picks one of three paths:
   - `thread-immediately` + `TRADES_CHANNEL_ID` set + proposer has `discordId` → bot creates a private thread, adds both users in parallel (`Promise.all` — fail-fast kicks into DM fallback, `api/trades.ts:302-328`), posts the proposal message.
