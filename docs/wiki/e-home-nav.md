...
- **Module (in HomeView)** — one of four parallel panels on the signed-in dashboard: TradesModule, WishlistModule, BinderModule, CommunitiesModule. All use the shared `ModuleSection` chrome. A fifth — `StoresModule` — is a dimmed placeholder for Phase 4 v2+.

## File map

### Home surfaces

**`src/components/HomeView.tsx`** — Signed-in dashboard. Four modules (Trades / Wishlist / Binder / Communities) plus the `NeedsResponseCallout` at the top and a `StoresModule` placeholder footer. Also exports the `BADGE_TONES` record + `stateBadgeSpec()` that map `TradeRowState → tone` (the canonical trade-state visual language other areas consume).
...
### 4-module dashboard (`HomeView.tsx`)
...
│ Your wishlist (action)   │ My Communities (resource)    │
├──────────────────────────┴──────────────────────────────┤
│ My Stores (placeholder, full width, dashed border)      │
└─────────────────────────────────────────────────────────┘
```

Mobile collapses to a single column in priority order: Trades, Wishlist, Binder, Communities, Stores.

Visual language:
- `ModuleSection` (`src/components/HomeView.tsx:899`) is the shared chrome — rounded-xl, `bg-space-800/20` wash, uppercase tracked-label header with icon, optional right-aligned action link.
...
- Gold column-left border is reserved for the callout ONLY — the four module panels use neutral space-700 borders so the callout visually dominates.
...
- **4-bar mutex on trade-builder** — `EditBar`, `CounterBar`, `ProposeBar`, and `AutoBalanceBanner` are rendered via a ternary chain at `src/App.tsx:575`. Only one can mount at a time, and the routing is inherently mutually-exclusive (editing ≠ countering ≠ proposing), but the chain is growing and bugs like "ProposeBar + AutoBalanceBanner flash together on a pushState" would be easy to introduce. Queued as UX-A2; cross-ref [`c-trade-builder.md`](./c-trade-builder.md).
- **Communities module competes with trading loop** — on Home, `CommunitiesModule` sits in the right column next to Binder. It's a discovery surface, not a trade-completion surface, and there's a risk of it pulling users sideways out of the response/response-cycle flow. Queued as UX-A4.
- **Ghost → real-user merge banner (UX-A5)** — the OAuth callback silently rewrites ghost sessions to the real user (api/auth.ts); no UI acknowledgment. A returning ghost who signs in gets their sessions but no "Your trades have been linked to your account" surface. `GhostHomeView` is where the banner would live (above or inside the greeting card). Currently un-shipped.
