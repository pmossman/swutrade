...
- **Module (in HomeView)** — one of three parallel panels on the signed-in dashboard: `TradesModule`, `WishlistModule`, `BinderModule`. All use the shared `ModuleSection` chrome. A fourth — `StoresModule` — is a dimmed placeholder for Phase 4 v2+. A `CommunitiesModule` used to live here too; removed in UX-A4 (2026-04-21) because it competed with the trading loop without fitting the primary path.

## File map

### Home surfaces

**`src/components/HomeView.tsx`** — Signed-in dashboard. Three modules (Trades / Wishlist / Binder) plus the `NeedsResponseCallout` at the top and a `StoresModule` placeholder footer. Also exports the `BADGE_TONES` record + `stateBadgeSpec()` that map `TradeRowState → tone` (the canonical trade-state visual language other areas consume). A fourth CommunitiesModule used to render here; removed in UX-A4.
...
A fourth `CommunitiesModule` used to occupy the right column alongside Binder. Removed in UX-A4 (2026-04-21): it competed with the trading loop on a surface where it wasn't load-bearing, and its destination already lives at `/?community=1` via the NavMenu entry.
...
- **~~Communities module competes with trading loop~~ — shipped UX-A4 (2026-04-21)**. `CommunitiesModule` removed from HomeView; discovery lives at `/?community=1` via NavMenu entry.
