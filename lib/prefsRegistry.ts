...
export const PREF_DEFINITIONS: ReadonlyArray<PrefDefinition> = [
  ...
  // Session-lifecycle DM toggles (Phase B). Defaulted ON so users
  // hear about activity in their trades; opt-out per event for
  // people who'd rather pull from the web than receive pings.
  definePref({
    key: 'dmSessionInvited',
    scope: { kind: 'self' },
    column: 'dmSessionInvited',
    type: { kind: 'boolean' },
    label: 'Trade invites',
    description: 'DM me when someone starts a trade with me directly.',
    default: true,
    surfaces: ['web', 'discord'],
    section: 'notifications',
    discord: { order: 20 },
  ...
  definePref({
    ...
    type: { kind: 'boolean' },
    label: 'Trade settled',
    description: 'DM me when both sides confirm and a trade settles.',
    default: true,
    surfaces: ['web', 'discord'],
    section: 'notifications',
    discord: { order: 22 },
  ...
  definePref({
    ...
    scope: { kind: 'self' },
    column: 'dmSessionDeclined',
    type: { kind: 'boolean' },
    label: 'Trade declined',
    description: 'DM me when a counterpart declines a trade I started.',
    default: true,
    surfaces: ['web', 'discord'],
    section: 'notifications',
    discord: { order: 24 },
